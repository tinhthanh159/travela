$(document).ready(function () {
    ...
    $("#register-form").on("submit", function (e) {
        ...
        // Vô hiệu hóa nút submit để ngăn ngừa việc nhấn nhiều lần
        ...
        var sqlInjectionPattern = /['"=;]/; // Mẫu kiểm tra ký tự đặc biệt
        ...
        if (!isValid) {
            ...
            $("#signup").prop("disabled", false); // Bật lại nút nếu có lỗi
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#register-form")
                    ...
                    .trigger("reset");
                $(".loader").hide();
                $("#signup").prop("disabled", false); // Bật lại nút sau khi thành công
            },
            error: function (xhr, textStatus, errorThrown) {
                toastr.error("Có lỗi xảy ra. Vui lòng thử lại sau.");
                $(".loader").hide();
                $("#register-form").removeClass("hidden-content");
                $("#signup").prop("disabled", false); // Bật lại nút nếu có lỗi server
            },
        });
    });
    ...
    $(".clear_filter").on("click", function (e) {
        ...
        // Gửi lại request AJAX không có filter
        $.ajax({
            url: filterToursUrl, // phải là URL load mặc định của danh sách tour
            type: "GET",
            data: {
                minPrice: 0,
                maxPrice: 20000000,
            ...
            ,
            error: function (xhr, status, error) {
                console.log("Lỗi khi tải danh sách tour:", error);
            }
        });
    });
    ...
    $(".btn-coupon").on("click", function (e) {
        ...
        if (couponCode === "DISCOUNT10") {
            ...
            // Disable + - input số lượng
            $(".quantity-btn, #numAdults, #numChildren").prop("disabled", true);
    ...
    $(".btn-remove-coupon").on("click", function () {
        ...
        $(".order-coupon input").val("");
        $(".summary-item:nth-child(3) .total-price").text("0 VNĐ");
    
        // Bật lại các input và nút số lượng
        $(".quantity-btn, #numAdults, #numChildren").prop("disabled", false);
    
        // Ẩn nút "Hủy", hiện lại "Áp dụng"
        $(".btn-remove-coupon").hide();
        $(".btn-coupon").show();
    
        toastr.info("Đã hủy mã giảm giá. Bạn có thể chỉnh lại số lượng.");
